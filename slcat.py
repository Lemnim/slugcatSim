...
class ItemMake:
    def __init__(self, name, itemType, effect, explain, canMake, makeScript):
        self.name = name
        ...
        items[name] = {"분류": itemType, "효과": effect, "설명": explain, "제작가능": canMake, "제작대사": makeScript}
        if canMake:
            recipes.append(name)
...
ItemMake("육개장 사발면", "음식", [("배고픔 증가", 3), ("호감도 증가", 50)], "슬러그캣은 육개장 사발면을 매워하면서 먹었다!\nʕx.xʔ: ♥ ♥!", True, "물을 끓이고.. 스프와 면을 넣은 후... 기다림의 시간...\nʕu.uʔ: z..!")
ItemMake("달콤바삭 애플파이", "음식", [("배고픔 증가", 2), ("호감도 증가", 50), ("도덕성 증가", 6)], "슬러그캣은 달콤하고 바삭한 애플파이를 정말 좋아했다!\nʕ>.<ʔ: ♥ ♥ ♥!", True, "사과를 졸이고.. 파이에 넣어서... 굽기!\nʕ●.●ʔ: ♥!")
ItemMake("카르마 꽃", "음식", [("치유", 1)], "슬러그캣은 카르마 꽃을 달여먹었다... \nʕ●.●ʔ: !", False, None)
ItemMake("보리차", "음식", [("치유", 0.5), ("도덕성 증가", 2)], "슬러그캣과 같이 따뜻한 보리차를 마셨다!\nʕu.uʔ: !", True, "보리를 볶고.. 물을 끓여서...\nʕ●.●ʔ: !")
ItemMake("스위트 포테이토 피자", "음식", [("배고픔 증가", 3), ("호감도 증가", 60), ("도덕성 증가", 4)], "슬러그캣은 친구들과 다 같이 스위트 포테이토 피자를 먹었다!\nʕ>.<ʔ: ♥ ♥ ♥!\nʕ●.●ʔ: ♥!\nʕu.uʔ: ♥ ♥!", True, "도우를 반죽하고.. 토핑을 올려서... 오븐에 굽기!\nʕ●.●ʔ: ♥!")

# 물건
ItemMake("분홍색 진주", "물건", [("호감도 증가", 200)], "반짝이는 분홍색 진주를 슬러그캣에게 선물했다!\nʕ>.<ʔ: ♥ ♥ ♥!", False, None)
ItemMake("착한 슬러그캣이 되는 법 제 1권", "물건", [("도덕성 증가", 10)], "슬러그캣에게 책을 읽어주었다.\nʕ●.●ʔ: !", False, None)
ItemMake("민들레 복숭아 핸드크림", "물건", [("치유", 0.6), ("호감도 증가", 70)], "슬러그캣에게 민들레 향기가 나는 핸드크림을 발라주었다!\nʕ●.●ʔ: !", True, "민들레 복숭아 오일을 넣고.. 깨끗한 물도 넣어서... 섞기!\nʕu.uʔ: !")
...
def look(moral, friship, haveRecipe:list):
...
def status():
    ...
    print(f"레시피: {haveRecipes}")
    print("#######################")

def use_item(item, hunger, evoLev, friship, moral, sick):
    print(item["설명"])
    for effect, change in item["효과"]:
        if effect == "배고픔 증가":
            hunger += change
...
def make(friship, haveItem:list, whatMake):
    friship -= 75
...
while True:
    ...
    if a == "먹이주기":
        ...
        morals, friendship, haveRecipes = look(morals, friendship, haveRecipes)
    elif a == "교육하기":
