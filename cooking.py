...
class Animal(Item):
    ...
    def breeding(self, money, haveAnimals:list):
        if haveAnimals.count(self.name) >= 2:
            print(f"{self.name}의 번식 확률 {self.breedPer * 100}%, 번식 비용 {self.animalPrice // 2}원")
            if check("번식"):
                money -= self.animalPrice // 2
...
Livestock("달걀", [Base.LIVESTOCK], 10)
Livestock("닭고기", [Base.LIVESTOCK], 25)

# 동물
Animal("소", [Base.LIVESTOCK], 50, 45, 150, items["우유"], items["소고기"], 0.2)
Animal("돼지", [Base.LIVESTOCK], 20, None, 70, None, items["돼지고기"], 0.6)
Animal("닭", [Base.LIVESTOCK], 15, 30, 30, items["달걀"], items["닭고기"], 0.4 )
